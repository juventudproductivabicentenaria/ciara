   ...
   # def mi_funcion(self, cr, uid, ids, algo, context=None):
    ...
    def validar_campo_numerico(self, cr, uid, ids):
        for d in self.browse(cr, uid, ids):
            for s in d.superficie_ids:
                espacio1=s.espacio
                espacio=s.espacio.replace(' ','')
                if len(espacio1)==len(espacio):
                    if s.espacio.isdigit()== False:
                        raise osv.except_osv(
                        ('Error!'),
                        (u'El Tipo de Superficie %s el campo hectarea debe ser\
                        numerico %s'% (s.tipo_id.nombre,s.espacio)))
                else:
                    raise osv.except_osv(
                        ('Error!'),
                        (u'El Tipo de Superficie %s el campo hectarea no debe de\
                        tener espacios en blanco "%s"'% (s.tipo_id.nombre,s.espacio)))
            return True
...
class superficie_fundo(osv.osv):
    ...
    _columns = {
        'superficie_id': fields.many2one('fundo', 'Fundo Zamorano', help='Superficies del Fundo Zamorano'),
        ...
        'tipo_id': fields.many2one('tipo_superficie', 'Tipo de Superficie', required=True, help='Se refleja el tipo de superficie (General, Aprovechable, Cultivable, Abrae)'),
    }
    ...
    def _check_unique_insesitive(self, cr, uid, ids, context=None):
        ...
        for self_obj in self.browse(cr, uid, ids, context=context):
            if self_obj.observacion and self_obj.observacion.lower() in  lst:
                if self.obj.tipo_id and self_obj.tipo_id.lower() in lst:
                    return False
                return False
        return True
...
class tipo_fundo(osv.osv):
    ...
    _columns = {
        'nombre': fields.char('Tipo de Fundo Zamorano', size=100, required=True, help='Tipo del Fundo, parceleros o conuqueros'),
        'active': fields.boolean('Activo'),
    }
    _defaults = {
        'active':True, 
